public class Sketch {
  ...
  public int linearSearch(int catNumToFind){
    for(int i=0;i<store.length;i++)
      if(store[i].getCatNum()==catNumToFind)return i;
    return -1;
  }
  public int recursiveLinearSearch(int catNumToFind, int startIndex){
    if(startIndex>store.length-1)return -1;
  ...
  public int binarySearch(int catNumToFind){
    ...
    while(low<high){
      if(store[guess].getCatNum()!=catNumToFind){
        if(store[guess].getCatNum() > catNumToFind){high=guess-1;guess=(high+low)/2;}
        else if(store[guess].getCatNum() < catNumToFind){low = guess+1;guess=(high+low)/2;}
        else return -1;}
      else
        return guess;
    }
  ...
  public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh){
    ...
    if(store[guess].getCatNum()!=catNumToFind&&nLow>nHigh)return -1;
    else if(store[guess].getCatNum()==catNumToFind&&nLow>nHigh)return guess;
    else if(store[guess].getCatNum()>catNumToFind)return recursiveBinarySearch(catNumToFind,nLow,guess-1);
    else return recursiveBinarySearch(catNumToFind,guess+1,nHigh);    
  }
  public void tester(){
